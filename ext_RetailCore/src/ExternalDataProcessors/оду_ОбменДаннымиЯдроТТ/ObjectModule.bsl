...
Функция ИсполняемаяФункцияПроверкиРегистрацииОбъектаОбменаПоОтборуИзНастроек(Источник, СтруктураНастроекОбмена)  Экспорт
	...
	ИначеЕсли СтруктураНастроекОбмена.МетаданныеНаименование = "Справочники" Тогда
		
		оду_ЭтоНовый = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Источник.ДополнительныеСвойства, "ЭтоНовый", Ложь);
				   
		Если  оду_ЭтоНовый = Истина Тогда
			оду_ЭтоНовыйОбъект = Новый Структура("оду_ЭтоНовыйОбъект", Истина); 
			СтруктураНастроекОбмена.Вставить("ДополнительныеСвойстваОбъекта", оду_ЭтоНовыйОбъект);
		ИначеЕсли СтруктураНастроекОбмена.ИмяТипаXML = "CatalogRef.Номенклатура" Тогда
			...
			Если ВДЗ.Следующий() Тогда
				...
				РезультатРегистрацииОбъектаОбмена = НЕ (ВДЗ.Код = Источник.Код И 
													ВДЗ.Наименование = Источник.Наименование И
													ВДЗ.Родитель = Источник.Родитель И
													ВДЗ.ПометкаУдаления = Источник.ПометкаУдаления И	
													ВДЗ.НаименованиеПолное = Источник.НаименованиеПолное);
			КонецЕсли; 
			
		КонецЕсли;
		
	ИначеЕсли СтруктураНастроекОбмена.МетаданныеНаименование = "Регистры сведений" Тогда
		Если СтруктураНастроекОбмена.ИмяТипаXML = "InformationRegisterRecordSet.СоставСегментаНоменклатуры" Тогда
			
			РезультатРегистрацииОбъектаОбмена = Ложь;
			Если Источник.Отбор.СпособДобавления.Значение = ПредопределенноеЗначение("Перечисление.СпособыДобавленияВСегмент.Вручную") Тогда
				Запрос = Новый Запрос;
				Запрос.Текст = 
					"ВЫБРАТЬ
					...
					|ГДЕ
					|	СегментыНоменклатуры.ПометкаУдаления = ЛОЖЬ
					|	И НЕ СегментыНоменклатуры.Наименование ПОДОБНО ""%ревизор%""
					|	И СегментыНоменклатуры.Ссылка = &Ссылка";
				
				Запрос.УстановитьПараметр("Ссылка", Источник.Отбор.Сегмент.Значение);
				
				РезультатЗапроса = Запрос.Выполнить();
				РезультатРегистрацииОбъектаОбмена = НЕ РезультатЗапроса.Пустой();	
			КонецЕсли;			
	...
	//		Если ВДЗ.Следующий() Тогда
	//			
	//			РазрешенаРегистрация = НЕ (ВДЗ.Код = Источник.Код И 
	//												ВДЗ.Наименование = Источник.Наименование И
	//												ВДЗ.Родитель = Источник.Родитель И
	//												ВДЗ.ПометкаУдаления = Источник.ПометкаУдаления И	
	//												ВДЗ.НаименованиеПолное = Источник.НаименованиеПолное);
